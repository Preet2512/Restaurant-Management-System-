...
{
	$email=$_GET['email'];
    $sql=mysqli_query($mysqli,"SELECT M_ID,Password FROM Manager WHERE Email='$email'");
    ...
    if(strcmp($_POST['userid'],$chck_id)==0)
	{
	    ...
	    {
		    $sql1=mysqli_query($mysqli,"UPDATE Manager SET Password='$new_pass' WHERE M_ID='$userid'");
		    if($sql1){ 
                echo '<script>'; 
                echo 'alert("Password Reset Successful.");'; 
                echo 'window.location.href = "manager_login.php";';
                echo '</script>';
            }
            else
