...
<!-- End vertical navbar -->
 ...
 <h2 class="display-4 text-white">Reservations made at your restaurant.</h2>
  <p class="lead text-white mb-0">Wow! So many reservations! You're gonna be busy now!! Manage your reservations here!</p>
  <div class="separator"></div>
<div class="row text-white">
   
    <?php
    $sql1=mysqli_query($mysqli,"SELECT * FROM Reservation");

    echo "<table class='table table-dark table-hover' >";
    echo "<thread>";
    echo "<tr>";
    echo "<th>Reservation Code</th><th>Customer Name</th><th>Table Type</th><th>Guests</th><th>Date</th><th>Time Slot</th><th>Phone Number</th><th>Restaurant</th><th>Status</th><th>Action</th>";
    echo "</tr>";
    echo "</thread>";
    echo "<tbody>";
    while($row=$sql1->fetch_assoc()) {
        echo "<tr>";
        echo "<td>".$row['Reserve_code']."</td><td>".$row['Name']."</td><td>".$row['Table_type']."</td><td>".$row['Num_guests']."</td><td>".$row['R_date']."</td><td>".$row['Time_slot']."</td><td>".$row['Phone_no']."</td><td>".$row['Res_Name']."</td>";
        $status=$row['Status'];
        if($status=="" or $status=="NULL")
        {
        echo "<td><button type='button' class='btn btn-info'><span class='fa fa-bars' aria-hidden='true'></span> Reserved</button></td>";
          }
           if($status=="arrived")
         {
        echo "<td><button type='button' class='btn btn-warning'><span class='fa fa-cog fa-spin' aria-hidden='true'></span> Arrived</button></td>";
          }
        if($status=="paid")
          {
        echo "<td> <button type='button' class='btn btn-primary'><span class='fa fa-check-circle' aria-hidden='true'></span> Completed</button></td>"; 
        } 
        if($status=="cancel")
          {
        echo "<td> <button type='button' class='btn btn-danger'><span class='fa fa-times' aria-hidden='true'></span> Not Arrived</button></td>"; 
        } 																								
				echo "<td>";
						echo "<a href='update_reserve.php?res_upd=".$row['Reserve_id']."' class='btn btn-info btn-flat btn-addon btn-sm m-b-10 m-l-5'><i class='fa fa-edit'></i></a>
						</td>";
        echo "</tr>";
    }
