...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit_email"])) {
	$result = mysqli_query($mysqli,"SELECT * FROM Customer WHERE Email='" . $_POST["email"] . "'");
	...
	if($count>0) {
		...
		if($mail_status == 1) {
			$result = mysqli_query($mysqli,"INSERT INTO OTP_Expiry(OTP,Is_Expired,Created_at) VALUES ('" . $otp . "', 0, '" . date("Y-m-d H:i:s"). "')");
			if(!empty($result)) {
				$success=1;
			}
		}
...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit_otp"])) {
	$result = mysqli_query($mysqli,"SELECT * FROM Otp_Expiry WHERE OTP='" . $_POST["otp"] . "' AND Is_Expired!=1 AND NOW() <= DATE_ADD(Created_at, INTERVAL 24 HOUR)");
	$count  = mysqli_num_rows($result);
	if(!empty($count)) {
	...
	<span class="error" style="color: #FF0000; margin-left: 10px;"><?php echo $error_message; ?></span>
		...
		?>
			   ...
			   <header class="text-center">
                    <p><strong><center><h3>Enter OTP</h3></center><strong></p>
					<p><strong><center><h4>Check your mail for the OTP</h4></center><strong></p>
                </header>
