...
<!-- End vertical navbar -->
 ...
 <h2 class="display-4 text-white">Your Restaurants Page</h2>
  <p class="lead text-white mb-0">You can see all your restaurant details here!!</p>
  <div class="separator"></div>
<div class="row text-white">
   
    <?php
    $sql1=mysqli_query($mysqli,"SELECT * FROM Restaurant");

    echo "<table class='table table-dark table-hover' >";
    echo "<thread>";
    echo "<tr>";
    echo "<th>Restaurant ID</th><th>Restaurant Category</th><th>Restaurant Name</th><th>Restaurant Email</th><th>Restaurant Phone</th><th>Restaurant Address</th>";
    echo "</tr>";
    echo "</thread>";
    echo "<tbody>";
    while($row=$sql1->fetch_assoc()) {
        $sql2=mysqli_query($mysqli,"SELECT Category_Name FROM Food_Category WHERE Category_ID='$row[Category_ID]'");
        while($row1=$sql2->fetch_assoc()) {
        echo "<tr>";
        echo "<td>".$row['Res_ID']."</td><td>".$row1['Category_Name']."</td><td>".$row['Res_Name']."</td><td>".$row['Res_Email']."</td><td>".$row['Res_Phone']."</td><td>".$row['Res_Address']."</td>";
        echo "</tr>";
    }
  }
  echo "</tbody>";
