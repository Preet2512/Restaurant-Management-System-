...
<!-- End vertical navbar -->
 ...
 <h2 class="display-4 text-white">Your Food Items Page</h2>
  <p class="lead text-white mb-0">Manage your food items here!</p>
  ...
  <div class="row text-white">
    ...
    for($i=1;$i<=$count;$i++) {
    $sql1=mysqli_query($mysqli,"SELECT * FROM Restaurant WHERE Res_ID='$i'");
    $sql2=mysqli_query($mysqli,"SELECT * FROM Food WHERE Res_ID='$i'");
    while($row1=$sql1->fetch_assoc()) {
      $r_name=$row1['Res_Name'];
    }

    echo "<table class='table table-dark table-hover' >";
    echo "<thread>";
    echo "<tr>";
    echo "<th colspan='5' style='text-align: center'>".$r_name."</th>";
    echo "</tr>";
    echo "<tr>";
    echo "<th>Food_ID</th><th>Image</th><th>Food_Name</th><th>Price</th><th>Available</th><th>Features</th><th>Action</th>";
    echo "</tr>";
    echo "</thread>";
    echo "<tbody>";
    while($row=$sql2->fetch_assoc()) {
        echo "<tr>";
        echo "<td>".$row['Food_ID']."</td><td><img src='Photos/".$row['Image_path']."' width='50px' height='50px'></td><td>".$row['Food_Name']."</td><td>".$row['Price']."</td><td>".$row['Available']."</td><td>".$row['Features']."</td>";
        echo "<td><a href='delete.php?food_del=".$row['Food_ID']."' class='btn-danger btn-flat btn-addon btn-sm m-b-10 m-l-5'><i class='fa fa-trash-o' style='font-size:16px'></i></a>  
        <a href='update_food.php?food_upd=".$row['Food_ID']."' class='btn-info btn-flat btn-addon btn-sm m-b-10 m-l-5'><i class='fa fa-edit'></i></a>
       </td>";
        echo "</tr>";
    }
    echo "</tbody>";
