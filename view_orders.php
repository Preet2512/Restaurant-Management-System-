...
<div class="row text-white">
    ...
    <?php
    $sql1=mysqli_query($mysqli,"SELECT * FROM Orders");

    echo "<table class='table table-dark table-hover' >";
    echo "<thread>";
    echo "<tr>";
    echo "<th>User</th><th>Food Ordered</th><th>Quantity</th><th>Price</th><th>Delivery Address</th><th>Status</th><th>Registered Date</th><th>Action</th>";
    echo "</tr>";
    echo "</thread>";
    echo "<tbody>";
    while($row=$sql1->fetch_assoc()) {
        $sql2=mysqli_query($mysqli,"SELECT Address FROM Customer WHERE User_ID='$row[User_ID]'");
        while($row1=$sql2->fetch_assoc()) {
        echo "<tr>";
        echo "<td>".$row['User_ID']."</td><td>".$row['Order_Name']."</td><td>".$row['Quantity']."</td><td>".$row['Price']."</td><td>".$row1['Address']."</td>";
        $status=$row['Status'];
        ...
        echo "<td> <button type='button' class='btn btn-primary'><span class='fa fa-check-circle' aria-hidden='true'></span> Delivered</button></td>"; 
					...
					echo "<td>".$row['Date']."</td>";
				echo "<td>";
						echo "<a href='update_order.php?order_upd=".$row['Order_ID']."' class='btn btn-info btn-flat btn-addon btn-sm m-b-10 m-l-5'><i class='fa fa-edit'></i></a>
						</td>";
        echo "</tr>";
    }
  }
